...
class SettingsController extends ControllerBase {
    ...
    public function loginAction() {
        $log = $this->configComp->set('key', 'login')->set('structure', ['d_un', 'd_p'])->get();
        ...
        $curlObj->set('url', 'https://b2b.pwrs.ru/Captcha')->curlInit();
        $curlObj->setCookieFile('/' . IS::app()->getConfig('config')['publicDir'] . '/files/cookie.txt');
        $img = $curlObj->getHtml();
        $curlObj->close();
        file_put_contents(ROOTDIR . '/' . IS::app()->getConfig('config')['publicDir'] . '/files/images/captcha.jpg', $img);
        return new ViewBase([
            'log' => $log
        ]);
    }
